...
pub(crate) fn add_arm_extras(vm: &mut Vm, nvic_changed_hook: pcode::HookId) {
    ...
    vm.add_op_injector(
        "enableIRQinterrupts",
        move |_: &Arch, _, _, _, b: &mut BlockState| {
            b.pcode
                .push((primask_reg, Op::IntAnd, (primask_reg, !CPSR_IRQ_MASK_BIT)));
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, _, b: &mut BlockState| {
            b.pcode
                .push((primask_reg, Op::IntOr, (primask_reg, CPSR_IRQ_MASK_BIT)));
            b.pcode.push(pcode::Op::Hook(nvic_changed_hook));
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, _, b: &mut BlockState| {
            b.pcode
                .push((primask_reg, Op::IntAnd, (primask_reg, !CPSR_FIQ_MASK_BIT)));
            b.pcode.push(pcode::Op::Hook(nvic_changed_hook));
            // Changing this variable could cause an interrupt to be triggered, so terminate the
            // block here.
            true
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, _, b: &mut BlockState| {
            b.pcode
                .push((primask_reg, Op::IntOr, (primask_reg, CPSR_FIQ_MASK_BIT)));
            b.pcode.push(pcode::Op::Hook(nvic_changed_hook));
            false
        },
    );
    ...
    let current_sp_mode_is_psp = vm
        .cpu
        .arch
        .sleigh
        .add_custom_reg("curr_sp_mode_is_psp", 4)
        .unwrap();
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, output: pcode::VarNode, b: &mut BlockState| {
            ...
            b.pcode
                .push((output, Op::Copy, pcode::Value::Const(1, output.size)));
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, output: pcode::VarNode, b: &mut BlockState| {
            b.pcode
                .push((output, Op::IntAnd, (control.truncate(1), 0b1_u8)));
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, output: pcode::VarNode, b: &mut BlockState| {
            b.pcode
                .push((output, Op::IntEqual, (current_sp_mode_is_psp, 0_u32)));
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, output, b: &mut BlockState| {
            b.pcode
                .select(output, current_sp_mode_is_psp, other_sp_reg, sp_reg);
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, inputs: pcode::Inputs, _, b: &mut BlockState| {
            ...
            b.pcode
                .select(other_sp_reg, current_sp_mode_is_psp, val, other_sp_reg);
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, _, output, b: &mut BlockState| {
            b.pcode
                .select(output, current_sp_mode_is_psp, sp_reg, other_sp_reg);
            false
        },
    );
    ...
    vm.add_op_injector(
        ...
        move |_: &Arch, _, inputs: pcode::Inputs, _, b: &mut BlockState| {
            ...
            b.pcode
                .select(other_sp_reg, current_sp_mode_is_psp, other_sp_reg, val);
            false
        },
    );
}
