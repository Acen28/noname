...
fn build_models(models: &config::MmioModels) -> (Vec<Model>, Vec<u32>) {
    let bitextract = models.bitextract.values().map(|x| Model {
        pc: x.pc,
        addr: x.addr as u32,
        kind: ModelKind::Bitextract {
            size: x.size,
            left_shift: x.left_shift,
            mask: x.mask,
        },
    });
...
impl<I: IoMemory> IoMemory for FuzzwareMmioHandler<I> {
    fn read(&mut self, addr: u64, buf: &mut [u8]) -> MemResult<()> {
        ...
        let pc = unsafe {
            let ctx = (*self.uc_ptr)
                .ctx
                .cast::<crate::unicorn_api::Context>()
                .as_mut()
                .unwrap();
            let vm = &mut *ctx.vm;
        ...
        let key = if self.access_contexts {
            ((pc as u64) << 32) | addr
        } else {
            addr
        };
        ...
        if let Some(model) = self
            .models
            .iter()
            .find(|x| x.matches_context(pc as u64, addr))
        {
    ...
    fn write(&mut self, addr: u64, value: &[u8]) -> MemResult<()> {
        ...
        if let Some(model) = self
            .models
            .iter()
            .find(|x| x.addr == addr as u32 && (x.pc == pc || x.pc == 0xffffffff))
        {
    ...
    fn restore(&mut self, snapshot: &Box<dyn std::any::Any>) {
        let (passthrough, source) = snapshot
            .downcast_ref::<(Vec<u32>, Box<dyn std::any::Any>)>()
            .unwrap();
