...
impl<T> CortexmTarget<T> {
        ...
        T: 'static,
    {
        vm.cpu
            .mem
            .get_io_memory_mut(self.mmio_handler?)
            .as_mut_any()
            .downcast_mut::<T>()
    }
}
...
impl<I: IoMemory + 'static> CortexmTarget<FuzzwareMmioHandler<I>> {
    pub fn fuzzware_init(
        ...
        let nvic_handler = vm
            .cpu
            .mem
            .register_io_handler(FuzzwareNvicHandler { uc: uc_ptr });
        ...
        for (name, region) in &config.memory_map {
            ...
            if name == "nvic" && nvic_enabled {
                ...
                anyhow::ensure!(
                    vm.cpu
                        .mem
                        .map_memory_len(region.base_addr, region.size, nvic_handler),
                ...
                anyhow::ensure!(
                    vm.cpu
                        .mem
                        .map_memory_len(region.base_addr, region.size, mmio_handler),
        ...
        if let Some(path) = debug_info_path {
            vm.env_mut::<FuzzwareEnvironment>()
                .unwrap()
                .set_debug_info(path)?;
        } else {
        ...
        unsafe {
            ...
            // fuzzware.
            {
                ...
                ctx.timer = Some(icicle_fuzzing::timer::add_block_timer(
                    &mut *ctx.vm,
                    move |_, _| fuzzware::timer_countdown_expired(uc_ptr),
                ));
            };
        ...
        for (addr, patch) in &config.patch {
            ...
            let reg = vm.cpu.arch.sleigh.get_reg(&patch.register).ok_or_else(|| {
                anyhow::format_err!(
                    "Unknown register in `patch` for {addr:#x}: {}",
                    patch.register
                )
            })?;
...
fn write_raw_bytes(
    ...
    vm.cpu
        .mem
        .write_bytes_large(region.base_addr, &bytes[..len], perm::NONE)
        .map_err(|e| {
            anyhow::format_err!(
    ...
    for entry in reader {
        match entry.map_err(|e| anyhow::format_err!("invalid ihex file: {e}"))? {
            ihex::Record::Data { offset, value } => {
                ...
                vm.cpu
                    .mem
                    .update_perm(addr, value.len() as u64, perm)
                    .map_err(|e| {
...
impl<T> icicle_fuzzing::Runnable for CortexmTarget<T> {
    ...
    fn run(&mut self, vm: &mut icicle_vm::Vm) -> anyhow::Result<VmExit> {
        let (icicle_exit, fuzzware_exit) = self.with_uc_ptr(vm, |uc| {
            ...
            if let Some(fuzzware_exit) = vm
                .env_mut::<FuzzwareEnvironment>()
                .unwrap()
                .fuzzware_exit
                .take()
            {
...
impl<T> icicle_fuzzing::FuzzTarget for CortexmTarget<T> {
    ...
    fn initialize_vm(
        ...
        vm.env
            .as_mut_any()
            .downcast_mut::<FuzzwareEnvironment>()
            .unwrap()
            .init(&mut vm.cpu);
...
impl FuzzwareEnvironment {
    ...
    pub fn init(&mut self, cpu: &mut Cpu) {
        if let Some(reg) = cpu.arch.sleigh.get_reg("afl.prev_pc") {
            ...
            self.masking = CovMasking::HitCounts {
                prev_pc_var: reg.var,
            };
        }
        self.xpsr = cpu
            ...
            .sleigh
            .get_reg("xpsr")
            .expect("xpsr must be configured before init")
            .var;

        if let Some(hook) = cpu.get_hooks().iter_mut().position(|x| {
            x.data_mut::<icicle_fuzzing::coverage::EdgeHookData>()
                .is_some()
        }) {
            self.masking = CovMasking::ExactEdgeCov {
                hook: hook as pcode::HookId,
            }
        }
    }
...
impl icicle_vm::cpu::Environment for FuzzwareEnvironment {
    ...
    fn restore(&mut self, snapshot: &Box<dyn std::any::Any>) {
        let (saved_prev, fuzzware) = snapshot
            .downcast_ref::<(Option<u64>, FuzzwareSnapshot)>()
            .unwrap();
