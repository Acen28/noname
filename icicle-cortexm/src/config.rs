...
fn default_interrupt_trigger() -> IndexMap<String, Trigger> {
    [(
        ...
        Trigger {
            ...
            kind: TriggerKind::Dynamic {
                fuzz_mode: FuzzMode::Fuzzed,
            },
        },
    )]
    .into_iter()
        ...
        if let Ok(trigger_mode) = std::env::var("TRIGGER_FUZZ_MODE") {
            if let Some((_, trigger)) = config.interrupt_triggers.first_mut() {
                if trigger_mode.eq_ignore_ascii_case("round_robin") {
                    trigger.kind = TriggerKind::Dynamic {
                        fuzz_mode: FuzzMode::RoundRobin,
                    };
...
impl<'de> Deserialize<'de> for Permissions {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        str.parse()
            .map_err(|_| serde::de::Error::custom(format!("invalid permission bits: {str}")))
    }
}
...
pub fn cortexm_memory_map() -> IndexMap<String, Memory> {
    [
        (
            "ram".into(),
            Memory::new(0x20000000, 0x00100000, "rw-".parse().unwrap()),
        ),
        (
            "mmio".into(),
            Memory::new(0x40000000, 0x20000000, "rw-".parse().unwrap()),
        ),
        (
            "nvic".into(),
            Memory::new(0xe0000000, 0x10000000, "rw-".parse().unwrap()),
        ),
        (
            "irq_ret".into(),
            Memory::new(0xfffff000, 0x1000, "--x".parse().unwrap()),
        ),
    ]
    .into_iter()
    .collect()
