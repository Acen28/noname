...
pub fn generate_and_save(path: &Path, force: bool) -> anyhow::Result<()> {
    ...
    let parent_dir = path.parent().ok_or_else(|| {
        anyhow::format_err!(
            "unable to determine directory to save config for: {}",
            path.display()
        )
    })?;
    ...
    if config_file.exists() && !force {
        tracing::warn!(
            "{} already exists, skipping config generation.",
            config_file.display()
        );
        return Ok(());
    }
    ...
    let is_likely_config_file = path
        ...
        .and_then(|x| x.to_str())
        .map_or(false, |name| {
            name.contains("config") || name.ends_with(".yml")
        });

    let config = from_elf(&path).with_context(|| {
        let help = if is_likely_config_file {
            " (input looks like an existing config)"
        } else {
            ""
        };
...
pub fn from_elf(path: &Path) -> anyhow::Result<FirmwareConfig> {
    ...
    let ram_range = memory
        .get("ram")
        .map(|x| (x.base_addr..x.base_addr + x.size));
    ...
    for sym in obj_file.symbols() {
        ...
        if KNOWN_IVT_SYMBOLS.contains(&name) {
            ...
            if let Some((_, ivt_section)) = memory
                .iter_mut()
                .find(|(_, info)| info.memory_range().contains(&address))
            {
                found_ivt = true;
    ...
    if !found_ivt {
        ...
        if let Some(text_section) = elf
            .section_headers(endian, data.as_slice())?
            .iter()
            .find(|x| match x.strings(endian, data.as_slice()) {
