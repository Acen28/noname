        ...
        for id in 0..n_workers {
            ...
            s.spawn(move || {
                while let Some(binary) = {
                    ...
                    let Some(parent_dir) = binary.parent() else {
                    ...
                    let workdir = shared_workdir.join(
                        parent_dir
                            .file_name()
                            .expect("invalid file name for parent directory"),
                    );
                    ...
                    let Some(key) = get_groundtruth_key(&binary) else {
                    ...
                    match find_input_for_tests(config, tests) {
                        ...
                        Err(e) => output.extend(
                            tests
                                .iter()
                                .map(|x| TestResult::error(x.line, format!("{e}"))),
                        ),
                    }
                }
            });
        ...
        for entry in path_tracer.get_last_blocks(&mut vm) {
            checkers.retain_mut(|(case, checker)| {
                if checker.check(entry.pc) {
                    results.insert(
                        case.line,
                        TestResult::success(case.line, input_path.clone()),
                    );
                    return false;
                }
                true
...
fn parse_groundtruth_csv(
    ...
    for (i, line) in reader.lines().enumerate().skip(1) {
        ...
        // previous file.
        if !line.starts_with('\t') {
            let Some((filename, rest)) = line.split_once('\t') else {
