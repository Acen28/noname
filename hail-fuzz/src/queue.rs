...
impl<D> CorpusStore<D> {
    ...
    pub fn recompute_input_prioritization(&mut self) {
        ...
        for group in self.coverage_hits.values().filter(|x| !x.is_empty()) {
            let Some(&min) = group
                .iter()
                .min_by_key(|id| self.test_cases[**id].favored_metric())
            else {
                continue;
            };
...
impl CorpusStore<MultiStream> {
    pub fn maybe_save(&mut self, workdir: &std::path::Path) -> anyhow::Result<()> {
        ...
        std::fs::write(
            workdir.join("testcases.json"),
            serde_json::ser::to_vec(&testcases)?,
        )?;
        std::fs::write(
            workdir.join("metadata.json"),
            serde_json::ser::to_vec(&self.metadata)?,
        )?;
    ...
    fn add_to_global_metadata(&mut self, id: InputId, state: &State) {
        ...
        // prioritization.
        if !state.effective_is_crashing {
    ...
    fn remove_global_metadata(&mut self, id: InputId) {
        ...
        for (key, entry) in &entry.data.streams {
            ...
            let Some(stream_metadata) = self.metadata.streams.get_mut(key) else {
                continue;
            };
            stream_metadata.num_inputs -= 1;
            stream_metadata.total_bytes -= entry.bytes.len();
    ...
    pub fn disable_input(&mut self, id: InputId) {
        if id < self.test_cases.len() {
            self.remove_global_metadata(id);
            self.test_cases[id].metadata.disabled = true;
        }
    }

    pub fn collect_seed_and_descendants(&self, root_id: InputId) -> Vec<InputId> {
        use hashbrown::HashSet;
        let mut to_visit = VecDeque::new();
        to_visit.push_back(root_id);
        let mut visited = HashSet::new();
        let mut result = Vec::new();

        while let Some(id) = to_visit.pop_front() {
            if visited.contains(&id) || id >= self.test_cases.len() {
                continue;
            }
            visited.insert(id);
            result.push(id);
            ...
            for (idx, test_case) in self.test_cases.iter().enumerate() {
                if test_case.metadata.parent_id == Some(id) && !visited.contains(&idx) {
    ...
    pub fn disable_seed_and_descendants(&mut self, root_id: InputId) -> Vec<InputId> {
        let ids = self.collect_seed_and_descendants(root_id);
        for &id in &ids {
            if id < self.test_cases.len() && !self.test_cases[id].metadata.disabled {
                self.disable_input(id);
            }
        }
        ids
    }

    pub fn count_exclusive_coverage_bits(&self, seed_ids: &[InputId]) -> usize {
        use hashbrown::HashSet;
        let disabled: HashSet<InputId> = seed_ids.iter().copied().collect();
        ...
        self.coverage_hits
            .values()
            .filter(|owners| {
                owners.iter().any(|id| disabled.contains(id))
                    && owners.iter().all(|id| disabled.contains(id))
            })
            .count()
    }

    pub fn is_seed_disabled(&self, id: InputId) -> bool {
        if id >= self.test_cases.len() {
            return true;
        }

        if self.test_cases[id].metadata.disabled {
            return true;
        }

        let mut current_id = id;
        let mut visited = HashSet::new();
        while let Some(parent_id) = self.test_cases[current_id].metadata.parent_id {
            if visited.contains(&parent_id) {
                break; 
            }
            visited.insert(parent_id);
            if parent_id >= self.test_cases.len() {
                break;
            }
            if self.test_cases[parent_id].metadata.disabled {
                return true;
            }
...
pub(crate) struct InputMetadata {
    ...
    /// Whether this was a hanging input (timeout).
    pub is_hang: bool,
    /// Whether this input has been disabled (e.g., due to being a descendant of a True Crash).
    pub disabled: bool,
    /// CFG edges learned by this input: Vec<(source_addr, dest_addr)>
    pub learned_cfg_edges: Vec<(u64, u64)>,
    pub native_crash_count: u32,
}
...
impl InputSource for CoverageQueue {
    fn next_input(&mut self, corpus: &CorpusStore<MultiStream>) -> Option<InputId> {
        loop {
            ...
            if corpus.is_seed_disabled(id) {
                continue;
            }

            if id >= corpus.test_cases.len() {
                continue;
            }
            let metadata = &corpus.test_cases[id].metadata;

            if metadata.is_crashing {
                continue;
            }
...
impl InputQueue for CoverageQueue {
    fn add_if_interesting(
        ...
        if !state.is_potential_crash && state.effective_is_crashing {
            return None;
        }
...
pub(crate) fn current_state_is_favored(
    ...
    for idx in &state.hit_coverage {
        ...
        let Some((id, metadata)) = best else {
