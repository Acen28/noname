...
pub struct MultiStream {
    ...
    /// Optional taint tracker for Validator (only set during REPLAY in validator thread)
    pub taint_tracker: Option<std::sync::Arc<std::sync::Mutex<crate::validator::TaintTracker>>>,
}
...
impl Clone for MultiStream {
    fn clone(&self) -> Self {
        Self {
            ...
            taint_tracker: self.taint_tracker.clone(),
        }
    }
    ...
    fn clone_from(&mut self, source: &Self) {
        ...
        self.taint_tracker = source.taint_tracker.clone();
    }
}
...
impl MultiStream {
    ...
    /// Set taint tracker for Validator (only used during REPLAY in validator thread)
    pub fn set_taint_tracker(
        &mut self,
        tracker: Option<std::sync::Arc<std::sync::Mutex<crate::validator::TaintTracker>>>,
    ) {
        self.taint_tracker = tracker;
    }
    ...
    pub fn next_bytes(&mut self, addr: StreamKey, size: usize) -> Option<&[u8]> {
        ...
        let buf = stream
            .bytes
            .get(stream.cursor as usize..stream.cursor as usize + size)?;
    ...
    pub fn trim(&mut self) {
        self.streams
            .values_mut()
            .for_each(|x| x.bytes.truncate(x.cursor as usize));
    }

    pub fn snapshot_cursors(&self) -> Vec<(u64, u32)> {
        self.streams
            .iter()
            .map(|(key, value)| (*key, value.cursor))
            .collect()
    }
...
impl IoMemory for MultiStream {
    fn read(&mut self, addr: u64, buf: &mut [u8]) -> MemResult<()> {
        let data = self
            .next_bytes(addr, buf.len())
            .ok_or(MemError::ReadWatch)?;
        buf.copy_from_slice(data);

        // Record taint information if tracker is set (Validator REPLAY mode)
        if let Some(tracker) = &self.taint_tracker {
            if let Some(stream_key) = self.last_read {
                if let Some(stream) = self.streams.get(&stream_key) {
                    let offset = stream.cursor.saturating_sub(buf.len() as u32) as usize;
                    let size = buf.len();
                    // Get current instruction count from VM (if available)
                    // Note: icount will be updated later, we use 0 as placeholder
                    let icount = 0; // Will be updated by Validator after execution
                    if let Ok(mut tracker) = tracker.lock() {
                        tracker.record_mmio_read(addr, stream_key, offset, size, icount);
                    }
                }
            }
        }
