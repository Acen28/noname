...
impl FuzzerStage for TrimStage {
    fn run(fuzzer: &mut Fuzzer, stats: &mut LocalStats) -> anyhow::Result<StageExit> {
        ...
        let Some(mut cursor) = MultiStreamTrimCursor::new(&fuzzer.state.input) else {
            return Ok(StageExit::Skip);
        };
        ...
        while let Some((stream_key, offset, len)) = cursor.get(&fuzzer.state.input) {
            ...
            let diverges = fuzzer.state.effective_is_crashing
                || !bits_to_keep
                    .iter()
                    .all(|bit| is_bit_set(current_bits, *bit));
            if diverges {
...
impl MultiStreamTrimCursor {
    ...
    fn get(&mut self, input: &MultiStream) -> Option<(StreamKey, usize, usize)> {
        while self.current_stream < self.streams.len() {
            ...
            let min_remove_size = usize::max(
                stream_len.next_power_of_two() / TRIM_END_STEPS,
                TRIM_MIN_BYTES,
            );
...
impl TrimLogger {
    fn new(input_id: InputId, workdir: &std::path::Path) -> Self {
        if icicle_fuzzing::parse_bool_env("LOG_TRIM")
            .unwrap()
            .unwrap_or(false)
        {
            Self {
