...
impl StageData for HavocStage {
    fn start(fuzzer: &mut Fuzzer) -> Result<Self, StageExit> {
        ...
        let (Some(id), data) = (fuzzer.input_id, &fuzzer.state.input) else {
    ...
    fn fuzz_one(&mut self, fuzzer: &mut Fuzzer) -> Option<VmExit> {
        ...
        if fuzzer.debug.havoc && !self.saved {
            let _ = std::fs::write(
                fuzzer
                    .workdir
                    .join(format!("queue/{}.havoc.bin", fuzzer.input_id.unwrap_or(0))),
...
impl HavocStage {
    ...
    fn havoc_v1(&mut self, fuzzer: &mut Fuzzer) {
        ...
        while mutations > 0 {
            ...
            let num_mutations = fuzzer
                .rng
                .gen_range(1..=mutations.min(max_mutations_for_stream));
            mutations -= num_mutations;
            for _ in 0..num_mutations {
                let mutation =
                    self.mutator
                        .havoc_bytes(&mut fuzzer.rng, dict, bytes, key, &fuzzer.corpus, 0);
...
fn log2_max_mutations(fuzzer: &Fuzzer) -> u32 {
    let max_find_gap = fuzzer
        .input_id
        .map(|id| fuzzer.corpus[id].metadata.max_find_gap)
        .unwrap_or(0);
    match max_find_gap {
        ..=1000 => 3,
        ..=10000 => 4,
...
impl HavocMutator {
    pub(crate) fn new() -> Self {
        ...
        Self {
            mutation_distr: WeightedAliasIndex::new(weights).unwrap(),
        }
    }

    #[allow(unused)]
    fn random_weights<R: Rng>(rng: &mut R) -> Self {
        let weights = ALL_MUTATIONS
            .iter()
            .map(|_| rng.gen_range(0..100))
            .collect();
        let Ok(mutation_distr) = WeightedAliasIndex::new(weights) else {
