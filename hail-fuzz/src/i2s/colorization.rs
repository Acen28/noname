...
impl ColorizationStage {
    ...
    fn fuzz_one(&mut self, fuzzer: &mut Fuzzer) -> Option<()> {
        ...
        range
            .get_mut(input)
            .copy_from_slice(range.get(&self.mutated_input));
        ...
        let diverged =
            fuzzer.state.effective_is_crashing || fuzzer.vm.cpu.icount != self.original_icount;
        ...
        if diverged {
            ...
            range
                .get_mut(&mut fuzzer.state.input)
                .copy_from_slice(range.get(&original_input.data));
    ...
    fn end(&mut self, fuzzer: &mut Fuzzer) {
        if fuzzer.debug.cmplog && fuzzer.global.is_main_instance() {
            let _ = std::fs::write(
                &fuzzer
                    .workdir
                    .join(&format!("queue/{}.colorized.bin", self.input_id)),
