...
pub(crate) fn capture_comparisons(fuzzer: &mut Fuzzer) -> Option<(VmExit, Comparisons)> {
    ...
    let Some(cmplog) = fuzzer.cmplog else {
        return None;
    };
...
impl Comparisons {
    ...
    pub(crate) fn get(&self, cursor: CmpCursor) -> Option<(u64, Operands<'_>)> {
        match cursor.array {
            0 => self
                .u8
                .get(cursor.offset)
                .map(|&(addr, operands)| (addr, operands.into())),
            1 => self
                .u16
                .get(cursor.offset)
                .map(|&(addr, operands)| (addr, operands.into())),
            ...
            4 => self
                .call
                .get(cursor.offset)
                .map(|(addr, (a, b))| (*addr, Operands::Bytes(a, b))),
            _ => None,
        }
    }
...
impl ReplacementFinder {
    ...
    pub(crate) fn get_value_to_replace<'a>(&self, dst: &'a [u8]) -> &'a [u8] {
        let len = self
            .replacement
            .len()
            .min(dst.len().saturating_sub(self.offset));
    ...
    fn prefix_match(&mut self, prefix: &[u8], size: usize) {
        ...
        if let Some(null_terminator) = prefix.iter().position(|x| *x == 0) {
            if null_terminator > size && prefix[..null_terminator].is_ascii() {
                self.extended_replacement
                    .extend_from_slice(&prefix[..null_terminator]);
            }
        }
    }
}
...
fn strided_prefix_match<const S: usize>(target: &[u8], x: &[u8]) -> usize {
    target
        .chunks_exact(S)
        .zip(x)
        .take_while(|(target, x)| target[0] == **x)
        .count()
}
...
fn either_strided_prefix_match<const N: usize>(target: &[u8], a: &[u8], b: &[u8]) -> PrefixMatch {
    match (
        strided_prefix_match::<N>(target, a),
        strided_prefix_match::<N>(target, b),
    ) {
        (0, 0) => PrefixMatch::None,
        (a, b) if a > b => PrefixMatch::A(a),
        (_, b) => PrefixMatch::B(b),
