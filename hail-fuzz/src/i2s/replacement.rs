...
use crate::{
    i2s::{
        ...
        log_cmplog_data, MAX_ONE_BYTE_MATCHES, MAX_ONE_BYTE_REPLACEMENTS,
    },
...
impl I2SRandomReplacement {
        ...
        if fuzzer.debug.cmplog && fuzzer.global.is_main_instance() {
            if let Some(cmplog) = fuzzer.cmplog {
                let _ = log_cmplog_data(
                    ...
                    &fuzzer
                        .workdir
                        .join(format!("cmplog/{input_id}.ext.cmplog.txt")),
                );
            }
        }
    ...
    pub fn random_replace(&mut self, fuzzer: &mut Fuzzer) -> Option<()> {
        ...
        for _ in 0..MAX_REPLACEMENT_ATTEMPTS {
            ...
            let offset = fuzzer
                .rng
                .gen_range(stream.cursor as usize..stream.bytes.len());
...
impl FuzzerStage for I2SReplaceStage {
    fn run(
        ...
        tracing::debug!(
            "I2S stage complete: {} replacements attempted",
            state.total_execs
        );
        if fuzzer.debug.cmplog && fuzzer.global.is_main_instance() {
            ...
            log_attempted_replacements(
                ...
                &fuzzer
                    .workdir
                    .join(format!("cmplog/{input_id}.replacements.csv")),
            )
            .unwrap();
        }
...
impl I2SReplaceStage {
    fn start(fuzzer: &mut Fuzzer) -> Result<Self, StageExit> {
        ...
        if fuzzer.debug.cmplog && fuzzer.global.is_main_instance() {
            ...
            if let Some(cmplog) = fuzzer.cmplog {
                log_cmplog_data(
                    ...
                    &fuzzer
                        .workdir
                        .join(format!("cmplog/{input_id}.cmplog_raw.txt")),
                )
                .unwrap();
            }
        }
        ...
        for (_, stream) in &mut base_input.streams {
            if stream.bytes.len() > 8 {
                ...
                let len: usize = fuzzer
                    .rng
                    .gen_range(1..=(stream.bytes.len() - start).min(32));
    ...
    fn next_replacement(&mut self, fuzzer: &Fuzzer) -> Option<()> {
        loop {
            ...
            let (stream_key, dst) = fuzzer
                .state
                .input
                .streams
                .iter()
                .nth(self.cursor.stream)
                .unwrap();
            ...
            let max_len = self
                .parent_prefix
                .get(stream_key)
                .map_or(fuzzer.features.max_i2s_bytes, |prefix| {
                    dst.bytes.len() - *prefix
                });
            self.cursor.finder.offset = self
                .cursor
                .finder
                .offset
                .max(dst.bytes.len().saturating_sub(max_len));
            ...
            loop {
                ...
                if let Some(addr) = found {
                    ...
                    self.replacement
                        .bytes
                        .clone_from(&self.cursor.finder.replacement);
                    self.replacement
                        .extended
                        .clone_from(&self.cursor.finder.extended_replacement);
                ...
                let start_offset = dst
                    .bytes
                    .len()
                    .saturating_sub(fuzzer.features.max_i2s_bytes);
    ...
    fn try_exec(
        ...
        let Some(exit) = fuzzer.execute() else {
            return Ok(false);
        };
...
impl Replacement {
    ...
    pub(crate) fn apply_extended(&mut self, input: &mut MultiStream) {
        ...
        self.tmp.extend(
            self.extended
                .iter()
                .flat_map(|&byte| std::iter::repeat(byte).take(self.stride)),
        );
