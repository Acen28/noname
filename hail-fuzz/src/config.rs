...
pub(crate) fn oracle_replay_enabled() -> bool {
    std::env::var("ORACLE_REPLAY").is_ok()
        || icicle_fuzzing::parse_bool_env("ORACLE_MODE")
            .ok()
            .flatten()
            .unwrap_or(false)
}
...
pub(crate) fn enable_multifuzz_crash_detection() -> bool {
    parse_bool_env("ENABLE_MULTIFUZZ_CRASH_DETECTION")
        .unwrap()
        .unwrap_or(true)
}

pub(crate) fn cold_start_execs() -> u64 {
    if oracle_replay_enabled() {
        return 0;
    }
    match std::env::var("COLD_START_EXECS") {
        Ok(val) => val.parse().unwrap_or_else(|_| {
            eprintln!(
                "[Config] ⚠️ Invalid COLD_START_EXECS value: {}, using default 100000",
                val
            );
            100000
        }),
...
impl EnabledFeatures {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let max_i2s_bytes = std::env::var("MAX_I2S_BYTES")
            .ok()
...
impl DebugSettings {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            cmplog,
            havoc,
            save_length_extension_metadata,
            save_input_coverage,
        })
    }
}
...
impl WorkdirLock {
    fn new(path: std::path::PathBuf) -> anyhow::Result<Self> {
        let file = std::fs::File::options()
            ...
            .with_context(|| {
                format!(
                    "Failed to obtain lock for working directory ({})",
                    path.display()
                )
            })?;
        Ok(Self {
...
pub(crate) fn init_workdir(config: &Config) -> anyhow::Result<WorkdirLock> {
    ...
    std::fs::create_dir_all(workdir.join("crashes").join("native_crashes"))
        .context("Error creating native_crashes directory")?;
    std::fs::create_dir_all(workdir.join("crashes").join("cfi_violations"))
        .context("Error creating cfi_violations directory")?;
    std::fs::create_dir_all(workdir.join("crashes").join("validated_crashes"))
        .context("Error creating validated_crashes directory")?;
