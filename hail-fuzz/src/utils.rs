...
pub(crate) fn get_stream_weights(
    ...
    let colorization_rates: Vec<_> = streams
        .iter()
        .map(|(key, len)| {
            color.get(key).map_or(1.0, |x| {
                *x as f64 / *len.min(&fuzzer.features.max_i2s_bytes) as f64
            })
        })
        .collect();
...
pub fn rand_pow2<R: Rng>(mut rng: R, log2_max: u32) -> u64 {
    2_f32
        .powf(1.0 + (rng.gen::<f32>() * (log2_max as f32 - 1.0)))
        .floor() as u64
}
...
pub fn count_parent_prefix(
    ...
    let Some(parent) = input.metadata.parent_id else {
        return HashMap::new();
    };
    ...
    for (addr, stream) in &input.data.streams {
        let Some(parent_stream) = parent_data.streams.get(addr) else {
            continue;
        };

        let shared_prefix = stream
            .bytes
            .iter()
            .zip(&parent_stream.bytes)
            .take_while(|(a, b)| a == b)
            .count();
