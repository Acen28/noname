...
fn init_coverage_array() -> Box<[u64; MAP_SIZE / 8]> {
    ...
    unsafe {
        ...
        if ptr.is_null() {
            panic!(
                "failed to allocate {} bytes for coverage array.",
                layout.size()
            )
        }
        Box::from_raw(ptr)
...
impl EdgeCountMap {
    ...
    pub fn with_strategy(vm: &mut Vm, config: &FuzzConfig, strategy: BucketStrategy) -> Self {
        ...
        let (start_addr, end_addr) = config
            .get_instrumentation_range(vm)
            .unwrap_or((0, u64::MAX));
...
impl Coverage for EdgeCountMap {
    ...
    fn restore_local(&mut self, vm: &mut Vm, snapshot: &Box<dyn Any>) {
        let snapshot = snapshot
            .as_ref()
            .downcast_ref::<EdgeCountSnapshot>()
            .unwrap();
        vm.cpu.trace[self.storage]
            .data_mut()
...
impl BlockCoverage {
    ...
    pub fn get_blocks(&self, vm: &mut Vm) -> Vec<u64> {
        ...
        if self.pack_bits {
            bit_iter(bitmap)
                .map(|x| mapping.get(x).map_or(0, |(addr, _)| *addr))
                .collect()
        } else {
            bitmap
                .iter()
    ...
    fn update_mapping_cache(&self, vm: &mut Vm) {
        let injector_mapping = match self.pack_bits {
            true => {
                &vm.get_injector_mut::<ExactBlockCoverageInjector>(self.injector)
                    .unwrap()
                    .mapping
            }
            false => {
    ...
    pub fn blocks_for(&self, vm: &mut Vm, bits: &[u32]) -> Vec<u64> {
        ...
        let mapping = self.mapping_cache.borrow();
        bits.iter()
            .map(|x| mapping.get(*x as usize).map_or(0, |(addr, _)| *addr))
            .collect()
    }
}
...
impl Coverage for BlockCoverage {
    ...
    fn restore_local(&mut self, vm: &mut Vm, snapshot: &Box<dyn Any>) {
        let snapshot = snapshot
            .as_ref()
            .downcast_ref::<BlockCoverageSnapshot>()
            .unwrap();
        ...
        if self.pack_bits {
            ...
            *out = serde_json::to_string(&data).unwrap();
            ...
            for (i, byte) in self.hit.iter().flat_map(|x| x.to_le_bytes()).enumerate() {
                let Some((addr, idx)) = mapping.get(i) else {
                    continue;
                };
