...
impl LoadResizeInjector {
    pub fn new(mmio_ref: IoHandler, multiblock: bool, optimize_upper_bits: bool) -> Self {
        Self {
            mmio_ref,
            optimize_upper_bits,
            resizer: LoadResizer::new(multiblock),
        }
    }
...
impl LoadResizer {
    ...
    pub fn resize_loads(
        ...
        for (idx, stmt) in pcode.instructions.iter().enumerate() {
            ...
            let Some(metadata) = self.state.loads.values().find(|x| x.offset == idx) else {
                new_block.push(*stmt);
                continue;
            };
    ...
    fn calculate_used_bits(
        ...
        let Some(lifter) = self.lifter.as_mut() else {
...
impl LocalLifter {
    ...
    fn get_or_lift_block<'a>(
        ...
        #[cfg(test)]
        match _code.map.get(&icicle_vm::cpu::BlockKey {
            vaddr,
            isa_mode: cpu.isa_mode() as u64,
        }) {
    ...
    fn lift_block(&mut self, cpu: &mut Cpu, ctx: u64, target: u64) -> Result<BlockGroup, Error> {
        ...
        Ok(self
            .lifter
            .lift_block(&mut lifter::Context::new(cpu, &mut self.code, target))?)
    }
}
    ...
    fn find_loads_and_uses(&mut self, block: &lifter::Block, track_loads: bool) {
        for (i, stmt) in block.pcode.instructions.iter().enumerate() {
            ...
            if track_loads && matches!(stmt.op, pcode::Op::Load(_)) {
                ...
                self.uses.entry(dst_expr).or_default().insert(
                    dst_expr,
                    Use {
