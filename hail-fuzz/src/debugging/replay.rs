...
fn replay_bench(mut vm: Vm, mut target: CortexmMultiStream, trials: u64) -> anyhow::Result<()> {
    ...
    let mut cursor_snapshot = target
        .get_mmio_handler(&mut vm)
        .unwrap()
        .source
        .snapshot_cursors();
    ...
    if let Err(e) = vm
        .jit
        .dump_jit_mapping("jit_table.txt".as_ref(), vm.env.debug_info().unwrap())
    {
    ...
    if let Some(bp) = std::env::var("REPLAY_FROM")
        .ok()
        .and_then(|addr| parse_addr_or_symbol(&addr, &mut vm))
    {
        vm.restore(&snapshot);
        target
            .get_mmio_handler(&mut vm)
            .unwrap()
            .source
            .restore_cursors(&cursor_snapshot);
...
fn replay_trace(mut vm: Vm, mut target: CortexmMultiStream) -> anyhow::Result<()> {
    ...
    if icicle_fuzzing::parse_bool_env("REPLAY_CMPLOG")?.unwrap_or(false) {
        let cmplog_ref = icicle_fuzzing::cmplog2::CmpLog2Builder::new()
    ...
    let print_count = match std::env::var("PRINT_LAST_BLOCKS").ok() {
        Some(c) => c
            .parse()
            .context("Failed to parse `PRINT_LAST_BLOCKS` environment variable")?,
        None => 10,
    };
    eprintln!(
        "[icicle] last blocks:\n{}",
    ...
    if icicle_fuzzing::parse_bool_env("SAVE_MMIO_READS")?.unwrap_or(false) {
        trace::save_mmio_reads(
            "mmio_reads.txt".as_ref(),
            &path_tracer.get_mmio_reads(&mut vm),
        );
    }
...
pub fn analyze_crashes(mut config: Config, path: &str) -> anyhow::Result<()> {
    ...
    let snapshot = vm.snapshot();
    for entry in input_src
        .list_files()
        .with_context(|| format!("failed list files in: {path}"))?
    {
        vm.restore(&snapshot);
        ...
        if !matches!(
            exit,
            VmExit::UnhandledException((ExceptionCode::Environment, _))
        ) {
...
impl InputSource {
    pub fn from_str(path: &str) -> Self {
        ...
        let (stem, compression) = match file_path
            .strip_suffix(".gz")
            .or_else(|| file_path.strip_suffix(".gzip"))
        {
...
fn read_within(
    ...
    let Some(mut entry) = archive
        .entries()?
        .flatten()
        .find(|x| x.path().map_or(false, |path| path == file_path))
    else {
        anyhow::bail!(
            "failed to find {} in {}",
            file_path.display(),
